...
> **Superseded key names (read before copying any listing below).** After this plan was executed,
> the branch was rebased onto a `main` carrying `ec6083b` ("Tab steps workspaces, arrows step
> windows"), which inverts the navigation keys: `Tab`/`Shift+Tab` now step **workspaces** and the
> **arrows** move the **window cursor**. Six tests were repaired accordingly (commit `0180a84`) and
> the spec's prose reconciled (`8d3887b`). The code listings below still carry the pre-rebase key
> names in their comments and `keyClick` calls — the shipped files are the source of truth, not
> these listings. Every behavioural claim in this plan is unchanged; only key names moved.
...
- `bash tests/ui/run.sh Peek::test_name` — one test. A **bare suite name silently fails**: `run.sh Peek` exits 1 with no output (a `qmltestrunner` filter quirk, not a suite problem). To run just the peek suite, run the whole UI tier and read its `Peek::` lines.
...
| `Overview.qml` | Hold state (`peeking`, `peekedKey`, `peekCancelled`), the `Space` branch, the force-clears, the disappearance check in `rebuild()`, the hint entry, the `PeekLayer` instantiation | 5, 7, 8, 9 |
...
```js
    ...
    //
    // (An earlier draft of this test normalised against the OUTER box and compared x centres
    // only, with a 3% tolerance. Measured against this code: correct code drifted 0.0076 on
    // that basis, a peek ignoring the `reserved` bar drifted 0.0061, and a peek dropping
    // `cellInset` drifted 0.0101 — all under 3%, two of them BELOW correct code. `reserved` is
    // purely vertical ([0,26,0,0]), so an x-only comparison could never see a peek that ignores
    // it, by construction. That version discriminated nothing; this one replaces it.)
...
- **Bind `armed`, or the lock gate is dead code.** Task 5 added `PeekLayer.armed` on review, to stop
  the mini-map firing one denied screencopy request per window over a lock-armed workspace. The
  property defaults to `false`, so an unbound version is silently permissive — it does not leak
  (the compositor denies regardless, `logic.js`' `lockSyncLua` installs `no_screen_share`
  unconditionally), but it does restore the wasted requests the gate exists to prevent.
- **Do NOT assign `z` on these tiles.** `WindowTile` owns `z: dragging ? 99999 : tileLayer * 10 + (hh.hovered ? 1 : 0)`, and an assignment from the instantiating scope *overrides* that binding — collapsing the layer spacing from 0/10/20 to 0/1/2 and putting a backdrop-layer fullscreen window (`layer: 0`) at risk of stacking wrongly against its neighbours, which would make Task 2's parity real in the data and wrong on screen. Set `tileLayer` and let the delegate compute its own `z`: that is exactly what the grid does (`Overview.qml`'s tile delegate sets `tileLayer: model.layer` and never touches `z`). Match it.
...
Run: `bash tests/ui/run.sh` (the whole UI tier; **a bare suite name does not work** — `run.sh Peek` exits 1 with no output, because `qmltestrunner` only accepts a `Suite::test_name` filter. Use `bash tests/ui/run.sh Peek::test_name` for one test.)
...
```qml
    ...
    // Three intention-named exits, not one boolean. An earlier draft had `peekClear(cancel)`, and
    // mutation testing proved the trap: `peekClear(false)` is NOT the inverse of `peekClear(true)`
    // (it leaves the cancel set), and no test caught a wrong argument at any call site.
    //
    // `peekAbort` is the only thing that ever sets `peekCancelled`, and it clears `peeking` in the
    // same call — which is what makes `peekCancelled && peeking` unreachable, and why `shown` is
    // just `peeking` with no cancel check.
    function peekRelease() {            // a real key-up: the hold is over and may start again
        peeking = false
        peekedKey = ""
        peekCancelled = false
    }
    function peekAbort() {              // the target went, a modal opened, focus left: stay shut
        peeking = false                 // until the key is physically released
        peekedKey = ""
        peekCancelled = true
    }
    function peekReset() {              // a fresh summon carries nothing over
        peeking = false
        peekedKey = ""
        peekCancelled = false
    }
...
**Files:**
- Modify: `Overview.qml` — `rebuild()`, `openMenu()`, the confirm-dialog open path, the key catcher's focus loss

> **API note, added after Task 7's review.** The single `peekClear(cancel)` this task was originally
> written against no longer exists: a boolean whose `false` value did *not* undo its `true` value was
> a silent trap, and mutation testing proved no test would catch a wrong argument. It is now three
> intention-named functions — `peekRelease()` (a real key-up: ends the hold and clears the cancel),
> `peekAbort()` (suppress re-opening until the key is physically released) and `peekReset()` (a fresh
> summon). **Every cancellation path in this task uses `peekAbort()`.** Line references below are
> also approximate: `Overview.qml` has grown by several hundred lines across Tasks 5 and 7, so locate
> functions by name, not by number.
- Test: `tests/ui/peek.qml`
...
Also: `peekedKey` is cleared by `peekAbort()`, so once cancelled this block stops running — which is what keeps a *second* disappearance from re-cancelling and is why the cancel flag survives on its own.
...
Run: `bash tests/ui/run.sh` (the whole UI tier; **a bare suite name does not work** — `run.sh Peek` exits 1 with no output, because `qmltestrunner` only accepts a `Suite::test_name` filter. Use `bash tests/ui/run.sh Peek::test_name` for one test.)
Expected: PASS, 27 real tests (29 reported rows, including the `initTestCase`/`cleanupTestCase` pair). The two negative controls must **still** pass — if `test_clearing_the_cursor_retargets_without_cancelling` now fails, the cancel is keyed on the resolved target instead of the model, which is the exact defect the spec exists to prevent.

- [ ] **Step 8: Run the full suite**

Run: `mise run test`
Expected: PASS. `openMenu`'s new line runs on every right-click in `tests/ui/actions.qml`; with no peek held, `peekAbort()` sets `peekCancelled` on a hold that does not exist, which is harmless — but confirm the actions suite is still green rather than assuming it.
