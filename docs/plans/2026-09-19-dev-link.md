...
**Status:** implemented 2026-09-19 (PR #27). Two defects found in review *after* the tasks below
were written are fixed in the shipped script, so where they differ, `scripts/dev-link.sh` is
authoritative: (1) `pids_now` lists `--all` and filters by `config_path` rather than using
`list -p`, which hides instances from a caller with no `WAYLAND_DISPLAY`; (2) `omarchy plugin
validate` runs only when linking, so a broken checkout manifest cannot block `--unlink` from
restoring the ordinary install.
...
- **`set -o pipefail` and a non-matching `grep`:** `before=$(pids_now)` where the pipeline's `grep`
  ...
  running shell.
- **A stub that re-reads a state file inside `(( ))` breaks twice over:** a missing file yields
  `((: n >  : arithmetic syntax error` on stderr *and* an unchanging pid, so cases fail for reasons
  unrelated to the script. Reproduced. Every stub reads its mode once, into a variable, and the
  fixture writes every state file it will read.
- **`systemctl --user show-environment` failing (no user bus) aborts the script** at
  `sig=$(session_var …)` under `set -euo pipefail`, skipping the offline link-and-skip path entirely.
  Reproduced; the `|| true` inside `session_var` is what restores it.
...
**Properties this must achieve:** (a) the real install is *moved*, never removed — no `rm -rf` of the
install path appears anywhere in this script, in any mode; only a symlink is ever `rm`'d. (b) The
install path must never end up pointing at itself. The guard is ordered first *and* compares physical
paths, because `$REPO` comes from `pwd -P` while `$INSTALL` is a spelled path — a string comparison of
the two would miss the case through any symlinked parent.

- [ ] **Step 4: Run the suite and watch the case pass**

Run: `bash tests/dev-link.sh`
Expected: `9 passed, 0 failed` — the three fixture self-checks and the six stash assertions.
...
**What these distinguish:** the installed-checkout case fails outright on the unguarded script — the
directory would be moved to the stash and `readlink` would report a path pointing at itself, which is
why both the "still a real directory" and the empty-`readlink` assertions are there. The re-point case
fails if `-n` is dropped from `ln -sfn` — the symlink
...
**Properties:** (a) the poll's deadline must bound *both* replacement and readiness — a shell whose
pid appears at 0.2s and whose IPC answers at 6s is a success, not a failure; (b) every `pids_now` call
is guarded by `|| true`, because an empty list aborts under
`pipefail` — verified on this machine; (c) `comm` requires lexicographically sorted input, which is
why `pids_now` ends in a bare `sort` and not `sort -n`; (d) the pid extraction must survive *compact*
...
- **Shared fixture:** Task 1 carries the three independence properties, opens with a self-check that
  proves the instance stub actually changes its answer, and states plainly what stubs cannot prove;
  Task 6 steps 4–5 close that gap against real output.
- **Reviewed 2026-09-19, four defects fixed:** a self-referencing symlink when run from the installed
  clone; a stub that never replaced the instance (and emitted an arithmetic error); readiness polling
  that allowed a single ping attempt after the pid appeared; and a failing `systemctl` aborting before
  the no-session branch. Each now has a case that goes red without its fix.
