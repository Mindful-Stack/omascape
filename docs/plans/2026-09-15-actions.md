...
[ "$A" != "null" ] || { echo "SKIP: no active window for the float probe"; exit 0; }
WAS=$(hyprctl -j activewindow | jq -r .floating)
hyprctl dispatch 'hl.dsp.window.float({ window = "address:'"$A"'", action = "on" })' >/dev/null
F1=$(hyprctl -j clients | jq -r ".[] | select(.address==\"$A\") | .floating")
hyprctl dispatch 'hl.dsp.window.float({ window = "address:'"$A"'", action = "on" })' >/dev/null
F2=$(hyprctl -j clients | jq -r ".[] | select(.address==\"$A\") | .floating")
hyprctl dispatch 'hl.dsp.window.float({ window = "address:'"$A"'", action = "'"$([ "$WAS" = true ] && echo on || echo off)"'" })' >/dev/null
echo "FACT float on→$F1 on-again→$F2 (both must be true; false on the second means it toggled)"
[ "$F1" = true ] && [ "$F2" = true ] || { echo "FAIL: action=on is a toggle, not an absolute state"; exit 1; }
