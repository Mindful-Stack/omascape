# Omascape — Actions Implementation Plan
...
**Branch:** `git checkout actions`. Work in `~/Source/omascape`.
...
**Live loop:** `LIVE="$HOME/.config/omarchy/plugins/se.mindfulstack.omascape"; cp logic.js WindowTile.qml FindBar.qml ContextMenu.qml Overview.qml "$LIVE"/ && omarchy restart shell`. Editing `Overview.qml` needs `omarchy restart shell`, not `rescanPlugins`. QML errors: `journalctl --user -t omarchy-shell -n 60`.
...
sleep 1.2
A=$(hyprctl -j clients | jq -r '.[] | select(.title=="omascape-probe-window") | .address')
...
Property this must achieve: each FACT line must be derived from a **fresh `hyprctl -j` read after the dispatch**, never from the value the script passed in, **and every fact a later task will build on must be asserted, not printed for a human to eyeball**. Cases 1 and 2 are both hard assertions: case 1 is the fact `tests/lua/mock_hl.lua` gets written from, so a line it prints unconditionally is worse than no line at all. Only the monitor cases are reported rather than asserted, because their correct values depend on which monitor is focused.

Three ways this script can print a true-looking line while proving nothing, all of which must be closed: an unguarded `hyprctl` subcommand (an unknown one prints `unknown request` and **exits 0**), a reply whose `error:` prefix is stripped off by the output filter, and a workspace that happens to be empty (`n=0` and a client count of `0` agree with each other and establish nothing). So guard the `repl` subcommand itself, fail on a non-zero status or an `error:` reply, and create the scratch window **before** case 1 so the probed workspace is guaranteed non-empty.

> The committed `tests/integration/actions-probe.sh` is the source of truth for this script. Its first pass shipped case 1 with no assertion at all and the code quality review caught it; the script was fixed in a follow-up round. Read the file, not this listing, where the two differ.
...
Run: `bash tests/ui/run.sh` (a bare suite name does not filter on this qmltestrunner build; use `Actions::test_name` for one test)
...
```qml
    ...
    function test_the_move_row_dispatches_a_workspace_move() {
        ...
        // seedTwoMonitors names the second monitor with a deliberately LONG connector
        // (Task 10 needed a menu wide enough to overflow the panel edge). Read the name
        // from the seed rather than hardcoding it.
        var otherMon = "HDMI-A-1-EXTERNAL-DISPLAY"
        var b = view.boxForWs(1)
        var p = view.testCanvas.mapToItem(view, b.x + 12, b.y + 12)
        rightPress(p.x, p.y)
...
Render it (`Logic.workspaceSwapLua(1, "eDP-1", "HDMI-A-1")`) and bump the guard by one. **Check the current value first** — earlier tasks have moved it and the number in this brief is stale.
...
- **The context menu's first painted frame, with animations disabled** (`motion: "off"` in
  `~/.config/omarchy/omascape.json`, so the fade is instant): right-click a tile and confirm the rows
  are laid out from the moment the menu appears, never stacked at the top for a frame. Qt's polish
  pass should make this structurally impossible, and an offscreen test cannot settle it either way —
  the zero row positions seen there are the test runner not pumping the event loop, not a real frame.
  One look with animations off closes the question.
- **Auto-repeat, which no offscreen test can cover** (QtTest's QML API cannot set `isAutoRepeat`):
  hold Ctrl+W down on a workspace with several windows — exactly one close must be dispatched, not
  one per repeat. Then open the context menu, hold a letter down through the dismissal, and release
  it — no query may start until a fresh press after the release.
