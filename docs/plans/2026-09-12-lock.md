# Omyview — Workspace Lock Implementation Plan
...
**Architecture:** Enforcement is compositor-side: named `no_screen_share` window rules matched on the workspace, created and reconciled by one idempotent `lockSyncLua(armed)` chunk, plus a layer rule excluding the overview itself and a share observer that only publishes `1`/`0` to a runtime state file (`lockInstallLua()`). omyview owns the armed set in `~/.config/omarchy/omyview-locks.json` (`OmyviewLocks.qml`, a watched atomic `FileView`), syncs it on load, change, `configreloaded`, open and toggle — never before the file has loaded — and renders badge/placeholder from the armed set and the watched state file. Rules stay enabled while armed; share detection is presentation only.
...
**Branch:** `git checkout lock`. Work in `~/Source/omyview`.
...
**Fixture facts:** `prepare.py` rewrites `Hyprland.` → `compositor.` (a QtObject stub with `monitors`, `workspaces`, `focusedWorkspace`, `focusedMonitor`, `commands[]` appended by `dispatch()`, and `signal rawEvent()`), `PanelWindow` → 1200×800 Item, injects `property alias test…` hooks, and **replaces** `OmyviewConfig.qml` with a stub (no Quickshell.Io). This plan adds a stub for `OmyviewLocks.qml` the same way. Quickshell's real `rawEvent(event)` passes an object with `name` and `data` strings; the stub's signal gains a `var event` parameter.
...
**Live loop:** `LIVE="$HOME/.config/omarchy/plugins/se.mindfulstack.omyview"; cp logic.js WindowTile.qml FindBar.qml Overview.qml OmyviewLocks.qml "$LIVE"/ && omarchy restart shell`. `journalctl --user -t omarchy-shell -n 60` for QML errors.
...
With the rule enabled on workspace `$WS` (re-run the eval line from the script without the trap), move a window there with `hyprctl dispatch movetoworkspacesilent $WS,address:0x…` and capture again: it must be black. Float a window on `$WS` (`hyprctl dispatch togglefloating`) and capture: black. Record both outcomes in the commit message. Disable and drop the rule afterwards (`_G.omyview_probe_rule:set_enabled(false); _G.omyview_probe_rule = nil`).
...
Property: `Mock.new()` per case is what gives each case a fresh `_G` (via `hl.__env`); a case that reused a mock would see the previous case's `omyview_lock`. The "reported once, names the selector" assertion depends on the mock's `hl.notification.create` capturing `text`, which it already does.
...
## Task 4: `OmyviewLocks.qml`, its fixture stub, Overview state, keys and sync points
...
`tests/ui/prepare.py`: change the stub compositor's `signal rawEvent()` to `signal rawEvent(var event)`; add `property alias testLocks: locks` to the alias block; and write the stub (next to the `OmyviewConfig.qml` stub):
...
- [ ] **Step 4: Create `OmyviewLocks.qml`**
...
Instance and sync points (near `OmyviewConfig { id: config }`):
```qml
...
```bash
git add OmyviewLocks.qml logic.js Overview.qml tests/ui/prepare.py tests/ui/run.sh tests/ui/lock.qml tests/tst_layout.qml
...
- **Lock for screen sharing:** `Ctrl+L` arms the selected workspace: its windows are black in
  ...
  disarm. The set is kept in `~/.config/omarchy/omyview-locks.json`.
```
...
1. `journalctl --user -t omarchy-shell -n 60` — no QML errors. `ls $XDG_RUNTIME_DIR/omyview/` shows `share-state` containing `0`.
2. SUPER+P, select a workspace with windows, Ctrl+L: the badge gains the lock; `cat ~/.config/omarchy/omyview-locks.json` shows it; `bash tests/integration/lock-probe.sh <that ws>` is not needed — instead `grim -o eDP-1 /tmp/x.png` after switching to that workspace: black.
3. Start a real share: `wf-recorder -f /tmp/rec.mp4` for ~10 s (or a browser call test page sharing the whole screen). While it runs: `cat $XDG_RUNTIME_DIR/omyview/share-state` → `1`; SUPER+P shows the placeholder on the armed box, thumbnails elsewhere. Stop the recording; state → `0`. Inspect the recording: the armed workspace black (including the first frames after switching to it), the overview absent.
4. `hyprctl reload`; within a second `hyprctl eval 'if not _G.omyview_lock then error("not reinstalled") end'` → `ok`; switch to the armed workspace and `grim`: black again.
...
capture. The armed set lives in ~/.config/omarchy/omyview-locks.json and is reconciled with the
...
- **Spec coverage:** verified facts → Task 1 probe script; compositor side (install/sync chunks, per-step guards, atomic publish, counter, mkdir once) → Task 2; layout flags and no tiles → Task 3; `OmyviewLocks.qml` (unresolved until load, atomic writes, malformed keeps previous, missing → `[]`), sync points (start, reload, open, load, toggle), persistence ordering (memory → compositor → disk), Ctrl+L, hint → Task 4; badge, glyph, numeral, no optimistic row, pending cleanup, find exclusion → Task 5; docs + live share + PR → Task 6.
- **Spec-conflict fix:** the spec's UI test list said "prepare.py pointing both FileViews at fixture-local temp files"; the plan stubs `OmyviewLocks.qml` instead (the fixture has no Quickshell.Io) and moves real-file behaviour to the live check. The spec's Tests section is updated to say so (see Task 4 commit — edit `docs/specs/2026-09-12-lock-design.md` "Offscreen UI" bullet: "with prepare.py replacing `OmyviewLocks.qml` by a stub whose `armed` starts null (`loadArmed`, `setSharing`, recorded `writes`)").
