# Omascape Motion — Implementation Plan
...
**Spec:** `docs/specs/2026-09-10-motion-design.md` (read it first). Branch: `animations` (worktree `~/Source/omascape-worktrees/animations`).
...
cp logic.js OmascapeConfig.qml WindowTile.qml Overview.qml SoftShadow.qml "$LIVE"/ && omarchy restart shell
```
...
Property: `parseConfig` must be the *only* place that knows the config keys, so `OmascapeConfig.apply` (Task 2) becomes three assignments and nothing else.
...
Copy + restart (Conventions). Confirm the shell log shows no QML error for `OmascapeConfig.qml` (`journalctl --user -u omarchy-shell -n 50` or the terminal the shell was restarted from) and that the picker still opens. Nothing consumes `motionEffective` yet; the wiring is verified in Task 3.

- [ ] **Step 4: Commit**

```bash
git add OmascapeConfig.qml README.md
...
Directly after `OmascapeConfig { id: config }` (line 49):
...
Copy + restart. SUPER+P: hover zoom, drag lift/release and arrow-key frame glide look as before (this task changes durations 100→90 and 120→90 only). Then set `"motion": "off"` in `~/.config/omarchy/omascape.json` (watched; no restart): reopen — the frame and hover snap. Remove the key again.
