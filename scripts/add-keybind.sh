...
#
# KEY defaults to "SUPER + A". The script appends one line to
...
if [[ -f $HYPR/bindings.lua ]]; then
  ...
  comment="--"
  line="o.bind(\"$KEY\", \"Workspace overview\", \"$ACTION\")"
  ...
  [[ -n $mods && -n $key ]] || fail "could not parse key '$KEY'; expected something like 'SUPER + A'"
  line="bind = $mods, $key, exec, $ACTION"
else
