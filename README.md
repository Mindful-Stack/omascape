...
also the padding at each side — tiles take whatever width is left), the cell size caps,
`cellInset`, the `minTileW`/`minTileH` clamps, and the fixed-pixel `cellSpacing`/`rowSpacing`
that apply only when `gapRatio` is removed or set to anything that is not a positive number.
After editing QML, run `omarchy restart shell` (see the note in Contributing about why a plain
rescan isn't enough).
